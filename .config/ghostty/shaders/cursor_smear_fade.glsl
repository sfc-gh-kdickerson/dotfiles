...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // const float TRAIL_WIDTH_FACTOR = 0.4; // Adjust this to your liking, e.g., 0.25 for thinner trails
    // vec2 trailWidth = currentCursor.zw * TRAIL_WIDTH_FACTOR;
    ...
    const float TRAIL_WIDTH_FACTOR = 0.4; // Adjust this to your liking
    vec2 trailWidth = currentCursor.zw * TRAIL_WIDTH_FACTOR;
    vec2 v0 = vec2(currentCursor.xy + vec2( trailWidth.x * vertexFactor, -trailWidth.y));
    vec2 v1 = vec2(currentCursor.xy + vec2( trailWidth.x * invertedVertexFactor, 0.0));
    vec2 v2 = vec2(previousCursor.xy + vec2(trailWidth.x * invertedVertexFactor, 0.0));
    vec2 v3 = vec2(previousCursor.xy + vec2(trailWidth.x * vertexFactor, -trailWidth.y));
